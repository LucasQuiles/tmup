...
export function claimTask(
  ...
  const claim = db.transaction(() => {
    ...
    const existing = db.prepare(
      "SELECT id FROM tasks WHERE owner = ? AND status = 'claimed' LIMIT 1"
    ...
    const task = db.prepare(
      "SELECT * FROM tasks WHERE owner = ? AND status = 'claimed' AND claimed_at = ?"
    ).get(agentId, now) as TaskRow | undefined;
    if (!task) {
...
export function claimSpecificTask(
  ...
  const claim = db.transaction(() => {
    ...
    const result = db.prepare(
      "UPDATE tasks SET status = 'claimed', owner = ?, claimed_at = ? WHERE id = ? AND status = 'pending'"
    ...
    const claimed = db.prepare('SELECT * FROM tasks WHERE id = ?').get(taskId) as TaskRow | undefined;
    if (!claimed) {
...
function hasAttestedValue(value: string | null): boolean {
  return value !== null && value.trim() !== '' && value !== 'unknown';
}

export function validateCompletionReceipt(
  task: TaskRow,
  attempt: TaskAttemptRow | undefined,
  acceptedEvidence: boolean,
  suppliedArtifactNames: Set<string>,
  declaredArtifactNames: Set<string>,
): void {
  ...
  if (missingArtifacts.length > 0) {
    throw new Error(
      `Task ${task.id} requires every declared produced artifact; missing: ${missingArtifacts.join(', ')}`
    );
  }
}
...
export function completeTask(
  ...
  const checksums: Array<{ name: string; path: string; checksum: string }> = [];
  if (artifacts) {
  ...
  const complete = db.transaction(() => {
    ...
    for (const art of checksums) {
      const artifact = db.prepare(
        'SELECT a.id FROM artifacts a JOIN task_artifacts ta ON a.id = ta.artifact_id WHERE a.name = ? AND ta.task_id = ? AND ta.direction = ?'
      ).get(art.name, taskId, 'produces') as { id: string } | undefined;
...
export function failTask(
  ...
  const fail = db.transaction(() => {
    ...
    const activeAttempt = db.prepare(`
      ...
      ORDER BY started_at DESC, rowid DESC
      LIMIT 1
    `).get(taskId, task.owner) as { id: string } | undefined;
    if (activeAttempt) {
      db.prepare(`
        UPDATE task_attempts
  ...
  const cancel = db.transaction(() => {
    ...
    if (cascade) {
      for (const depId of transitiveDeps) {
        const result = db.prepare(
          "UPDATE tasks SET status = 'cancelled', owner = NULL WHERE id = ? AND status NOT IN ('completed', 'cancelled')"
        ).run(depId);
        if (result.changes > 0) {
      ...
      for (const depId of transitiveDeps) {
        db.prepare(
          "UPDATE tasks SET status = 'needs_review' WHERE id = ? AND status NOT IN ('completed', 'cancelled')"
        ).run(depId);
      }
    }
