...
import type { SessionRegistry, RegistryEntry } from './types.js';
...
function canonicalizeProjectDir(dir: string): string {
  try {
...
export function readRegistry(): SessionRegistry {
  ensureStateRoot();
  ...
  for (const [key, entry] of Object.entries((parsed as Record<string, unknown>).sessions as Record<string, unknown>)) {
    const e = entry as Record<string, unknown>;
    if (e && typeof e === 'object' && typeof e.db_path === 'string' && e.db_path) {
      validated[key] = e as unknown as RegistryEntry;
    } else {
...
export function initSession(
  projectDir: string,
  sessionName?: string
): { session_id: string; db_path: string; reattached: boolean } {
...
export function getSessionDbPath(sessionId?: string): string | null {
