import type { Database, CreateMessageInput, MessageRow, TaskRow } from './types.js';
...
export function sendMessage(db: Database, input: CreateMessageInput): string {
  ...
  const run = db.transaction(() => {
    ...
    const count = db.prepare(
      'SELECT COUNT(*) as cnt FROM messages WHERE from_agent = ?'
    ).get(input.from_agent) as { cnt: number };
...
export function getInbox(
  ...
  if (markRead) {
    const readInbox = db.transaction(() => {
      const messages = db.prepare(query).all(agentId, broadcastAge) as MessageRow[];
  ...
  return db.prepare(query).all(agentId, broadcastAge) as MessageRow[];
}

export function getUnreadCount(db: Database, agentId: string): number {
  ...
  `).get(agentId, `-${BROADCAST_MAX_AGE_SECONDS}`) as { cnt: number };
  return row.cnt;
}
 ...
 */
export function pruneMessages(db: Database, maxAgeSeconds: number = PRUNE_MAX_AGE_SECONDS): number {
