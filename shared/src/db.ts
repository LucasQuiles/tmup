...
export function openDatabase(dbPath: string): Database {
  ...
  const ALLOWED_PRAGMAS: Record<string, 'integer' | 'string'> = {
    journal_mode: 'string',
  ...
  const contract = JSON.parse(fs.readFileSync(contractPath, 'utf-8')) as Record<string, number | string>;
  for (const [key, value] of Object.entries(contract)) {
    ...
    if (expectedType === 'integer') {
      ...
      if (!Number.isFinite(num)) throw new Error(`Invalid pragma value for ${key}: ${value}`);
      db.pragma(`${key} = ${num}`);
    } else {
      // String pragmas: only allow alphanumeric + underscore
      const str = String(value);
      if (!/^[a-zA-Z_]+$/.test(str)) throw new Error(`Invalid pragma value for ${key}: ${value}`);
      db.pragma(`${key} = ${str}`);
    }
  }
