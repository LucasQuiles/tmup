...
const LEAD_TRANSITIONS: Record<string, TaskStatus[]> = {
...
function _createTaskInner(db: Database, input: CreateTaskInput): string {
  ...
  if (input.produces) {
    for (const name of input.produces) {
      ...
      if (!artifact) {
        ...
        const existingProducer = db.prepare(
          "SELECT task_id FROM task_artifacts WHERE artifact_id = ? AND direction = 'produces'"
        ).get(artifact.id) as { task_id: string } | undefined;
        if (existingProducer) {
...
export function createTask(db: Database, input: CreateTaskInput): string {
  const create = db.transaction(() => _createTaskInner(db, input));
  return create.immediate();
}

export function createTaskBatch(db: Database, inputs: CreateTaskInput[]): string[] {
  const ids: string[] = [];
...
export function getActiveTaskForAgent(db: Database, agentId: string): TaskRow | null {
  return (db.prepare(
    "SELECT * FROM tasks WHERE owner = ? AND status = 'claimed' LIMIT 1"
  ).get(agentId) as TaskRow | undefined) ?? null;
}

export function updateTask(db: Database, taskId: string, updates: UpdateTaskInput): { ok: boolean; previous_status?: TaskStatus } {
  const run = db.transaction(() => {
    const task = db.prepare('SELECT * FROM tasks WHERE id = ?').get(taskId) as TaskRow | undefined;
    if (!task) throw new Error(`Task ${taskId} not found`);

    const result: { ok: boolean; previous_status?: TaskStatus } = { ok: true };
