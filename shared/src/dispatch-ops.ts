...
export function toDispatchReceipt(row: TaskAttemptRow): DispatchReceipt {
  return {
    attempt_id: row.id,
    task_id: row.task_id,
...
export function getDispatchReceipt(db: Database, attemptId: string): DispatchReceipt {
  const attempt = db.prepare('SELECT * FROM task_attempts WHERE id = ?').get(attemptId) as
...
): { task: TaskRow; receipt: DispatchReceipt } {
...
export function attestAttempt(
  db: Database,
  attemptId: string,
  input: AttemptAttestationInput,
): DispatchReceipt {
  requireNonEmpty(input.observed_model, 'observed_model');
  ...
  const attest = db.transaction(() => {
    ...
    if (result.changes === 0) {
      const existing = db.prepare('SELECT status, execution_outcome FROM task_attempts WHERE id = ?')
        .get(attemptId) as { status: string; execution_outcome: string | null } | undefined;
