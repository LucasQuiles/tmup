 ...
 */
export function getNextAction(db: Database, paneInfo: PaneInfo): NextAction {
  // 1. Tasks needing review (highest priority — represents a failure/retry scenario)
  const needsReview = db.prepare(
    "SELECT * FROM tasks WHERE status = 'needs_review' ORDER BY priority DESC LIMIT 1"
  ...
  const blocker = db.prepare(
    "SELECT * FROM messages WHERE type = 'blocker' AND read_at IS NULL ORDER BY created_at ASC LIMIT 1"
  ...
  const pendingCount = (db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'pending'").get() as { cnt: number }).cnt;
  const activeAgents = getActiveAgents(db);
  const claimedTaskCount = (db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'claimed'").get() as { cnt: number }).cnt;
  ...
  const incomplete = (db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status NOT IN ('completed', 'cancelled')").get() as { cnt: number }).cnt;
  ...
  const blockedCount = (db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'blocked'").get() as { cnt: number }).cnt;
  return {
    kind: 'waiting',
