    ...
    },
    {
        ...
        up: (db) => {
            ...
            if (emptySubjects.cnt > 0) {
                throw new Error(`Migration preflight failed: ${emptySubjects.cnt} tasks have empty subjects. ` +
      ...
      `).all();
            if (dupeOwners.length > 0) {
                ...
                throw new Error(`Migration preflight failed: agents with multiple active tasks: ${details}. ` +
      ...
      `).all();
            if (dupeProducers.length > 0) {
                throw new Error(`Migration preflight failed: ${dupeProducers.length} artifacts have multiple producers. ` +
