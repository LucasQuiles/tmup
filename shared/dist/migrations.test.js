...
describe('migration v4: colony support', () => {
    ...
    `).run();
        const row = db.prepare('SELECT bead_id, sdlc_loop_level, output_path, clone_dir, worker_type, bridge_synced FROM tasks WHERE id = ?').get('t1');
    ...
    it('accepts claude_code as worker_type', () => {
        const db = freshDb();
        runMigrations(db);
        db.prepare("INSERT INTO tasks (id, subject, worker_type) VALUES ('t1', 'claude task', 'claude_code')").run();
        const row = db.prepare('SELECT worker_type FROM tasks WHERE id = ?').get('t1');
    ...
    it('creates task_corrections table and it is functional', () => {
        ...
        const row = db.prepare('SELECT * FROM task_corrections WHERE task_id = ?').get('t1');
    ...
    it('allows NULL sdlc_loop_level for backward compatibility with non-colony tasks', () => {
        ...
        const row = db.prepare('SELECT sdlc_loop_level, bead_id FROM tasks WHERE id = ?').get('t1');
    ...
    it('creates idx_tasks_colony index', () => {
        const db = freshDb();
        runMigrations(db);
        const indexes = db.prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_tasks_colony'").all();
        expect(indexes).toHaveLength(1);
        db.close();
    });
...
describe('migration v5: dispatch receipt enforcement', () => {
    ...
    it('rejects invalid policy, outcome, and nullable boolean values', () => {
        ...
        const attemptColumns = db.prepare('PRAGMA table_info(task_attempts)').all().map((column) => column.name);
        ...
        expect(() => db.prepare("INSERT INTO tasks (id, subject, role_required) VALUES ('role-invalid', 'bad', 2)").run()).toThrow();
        expect(() => db.prepare("INSERT INTO tasks (id, subject, evidence_required) VALUES ('evidence-invalid', 'bad', -1)").run()).toThrow();
        expect(() => db.prepare("INSERT INTO tasks (id, subject, model_requirement) VALUES ('model-invalid', 'bad', 'guessed')").run()).toThrow();
        expect(() => db.prepare("INSERT INTO tasks (id, subject, execution_outcome) VALUES ('outcome-invalid', 'bad', 'completed')").run()).toThrow();
