...
describe('config.sh shell boundary', () => {
  ...
  describe('new config values from policy.yaml', () => {
    ...
    it('does not export obsolete codex model context overrides', () => {
      const configSource = fs.readFileSync(CONFIG_SH, 'utf-8');

      expect(configSource).not.toContain('CFG_CODEX_CONTEXT_WINDOW');
      expect(configSource).not.toContain('CFG_CODEX_AUTO_COMPACT');
      expect(configSource).not.toContain('CFG_CODEX_UNDO');
    });
...
case "$query" in
  ...
  '.codex.sandbox // "workspace-write"') echo 'workspace-write' ;;
  ...
  '.codex.subagents.max_depth // 1') echo '3' ;;
...
`);
      ...
      const result = runShell(`
        ...
        echo "$CFG_CODEX_MODEL:$CFG_CODEX_APPROVAL_POLICY:$CFG_CODEX_SANDBOX:$CFG_CODEX_NO_ALT_SCREEN:$CFG_CODEX_PLAN_FIRST:$CFG_CODEX_REASONING_EFFORT:$CFG_CODEX_REASONING_SUMMARY:$CFG_CODEX_PLAN_REASONING:$CFG_CODEX_VERBOSITY:$CFG_CODEX_SERVICE_TIER:$CFG_CODEX_TOOL_OUTPUT_LIMIT:$CFG_CODEX_WEB_SEARCH:$CFG_CODEX_HISTORY:$CFG_CODEX_SHELL_INHERIT:$CFG_CODEX_SHELL_SNAPSHOT:$CFG_CODEX_REQUEST_COMPRESSION:$CFG_CODEX_NOTIFICATIONS:$CFG_CODEX_BACKGROUND_TERMINAL_TIMEOUT:$CFG_CODEX_MAX_THREADS:$CFG_CODEX_MAX_DEPTH:$CFG_CODEX_JOB_TIMEOUT"
      `, { PATH: `${fakeBin}:/usr/bin:/bin` });
      expect(result).toBe('gpt-5.4-mini:on-request:workspace-write:false:false:xhigh:detailed:high:medium:flex:54321:cached:none:core:false:false:false:420000:3:3:2700');
    });

    it('caps codex worker limits', () => {
...
case "$query" in
  ...
  '.codex.subagents.max_depth // 1') echo '9' ;;
...
`);
      ...
      const result = runShell(`
        source "${CONFIG_SH}"
        echo "$CFG_CODEX_TOOL_OUTPUT_LIMIT:$CFG_CODEX_MAX_THREADS:$CFG_CODEX_MAX_DEPTH:$CFG_CODEX_JOB_TIMEOUT"
      `, { PATH: `${fakeBin}:/usr/bin:/bin` });
